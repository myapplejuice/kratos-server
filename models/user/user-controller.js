...
export async function getUserById(req, res) {
    ...
    let user = await User.fetchUsers(req.params.id)
...
export async function createUser(req, res) {
    ...
    let user = await User.newUser(req.body)
...
export async function updateUser(req, res) {
    ...
    let user = await User.adjustUser(req.params.id, req.body)

    if (!user) return res.status(404).json({ message: 'updateUser() Request fail! User update failed!' })
    return res.status(200).json({ message: 'updateUser() Request success!', user })
}
